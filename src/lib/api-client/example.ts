...
export const tradingApi = {
    ...
    try {
      if (markets) {
        ...
        for (const market of marketList) {
          try {
            const data = await api.get<{ticker: Ticker}>(`/api/v2/public/markets/${market}/tickers`);
            if (data && data.ticker) {
  ...
  getOrderBook: async (market: string, limit?: number): Promise<OrderBook> => {
    try {
      const endpoint = limit 
        ? `/api/v2/public/markets/${market}/order-book?asks_limit=${limit}&bids_limit=${limit}`
        : `/api/v2/public/markets/${market}/order-book`;
  ...
  getTrades: async (market: string, limit?: number): Promise<Trade[]> => {
    try {
      const endpoint = limit 
        ? `/api/v2/public/markets/${market}/trades?limit=${limit}`
        : `/api/v2/public/markets/${market}/trades`;
  ...
  getBalance: async (uid: string, currency: string, authToken: string): Promise<Balance> => {
    try {
      ...
      return await api.post<Balance>('/api/v2/management/accounts/balance', formData, {
        authToken,
        headers: {
  ...
  getBalances: async (currency: string, authToken: string, page?: number, limit?: number): Promise<Balance[]> => {
    try {
      ...
      return await api.post<Balance[]>('/api/v2/management/accounts/balances', formData, {
        authToken,
        headers: {
          'Content-Type': 'multipart/form-data',
        },
  ...
  getTimestamp: async (): Promise<number> => {
    try {
      return await api.post<number>('/api/v2/management/timestamp');
    } catch (error) {
      if (error instanceof APIError) {
        console.error(`API Error: ${error.message} (${error.status})`);
      }
